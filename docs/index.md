# Učimo elektroniku

## Osnove

- [Elektricitet](elektricitet.md)
- [Izvori elektriciteta](izvori-elektriciteta.md)
- [Komponente](komponente.md)
- [Elektronsko kolo](elektronsko-kolo.md)
- [Jedinice](jedinice.md)
- [Omov zakon](omov-zakon.md)
- [Statički elektricitet](staticki-elektricitet.md)
- [Alati i oprema](alati-i-oprema.md)
- [Mesto za rad](mesto-za-rad.md)

## Komponente

- [Otpornici](otpornici.md)
- [Kondenzatori](kondenzatori.md)
- [Diode](diode.md)
  - [LED diode](led-diode.md)
- [Tranzistori](tranzistori.md)
  - [MOSFET](MOSFET.md)
- [Žice](zice.md)
- [Baterije](baterije.md)
  - [Ispravljač](ispravljac.md)
- [Prekidači](prekidaci.md)
  - [Releji](releji.md)
- [Elektromotori](elektromotori.md)
- [Senzori](senzori.md)
- [Integrisano kolo](integrisano-kolo.md)
  - [Logička kola](logicka-kola.md)
- [Kalemovi i kristali](kalemovi-i-kristali.md)
- [Kombinacija kondenzator–otpornik](kombinacija-kondenzator-otpornik.md)

## Električna kola

- [Šeme](sheme.md)
- [Redna i uporedna kola](redna-i-uporedna-kola.md)
- [Razdelnik napona](razdelnik-napona.md)
- [Multimetar](multimetar.md)
  - [Merenja](merenja.md)
  - [Merenje struje i napona](merenje-struje-naponom.md)
- [Mikrokontroleri](mikrokontroleri.md)
- [Prototipska ploča](prototipska-ploča.md)
- [Štampana ploča](štampana-ploča.md)
- [Lemljenje](lemljenje.md)
- [Omotavanje žice](omotavanje-žice.md)
- [Formule](formule.md)

## Projekti

- [Autić](projekti/autic.md)
- [Detektor infracrvenog zračenja](projekti/detektor-infracrvenog-zračenja.md)
- [Detektor metala](projekti/detektor-metala.md)
- [Detektor strujnih vodova](projekti/detektor-strujnih-vodova.md)
- [Gitarsko pojačalo za slušalice](projekti/gitarsko-pojacalo-za-slusalice.md)
- [Ispravljač](projekti/ispravljac.md)
- [Jutarnji alarm](projekti/jutarnji-alarm.md)
- [Merač vlažnosti](projekti/merac-vlaznosti.md)
- [Piezo-električni bubanj](projekti/piezo-elektricni-bubanj.md)
- [Punjač akumulatora](projekti/punjac-akumulatora.md)
- [Radio-prijemnik](projekti/radio-prijemnik.md)
- [Sirena](projekti/sirena.md)
- [Solarni punjač](projekti/solarni-punjac.md)
- [Solarno baštensko svetlo](projekti/solarno-bastensko-svetlo.md)
- [Sunčev instrument](projekti/suncev-instrument.md)
- [Treptajuća svetla](projekti/trepcuca-svetla.md)
- [Treptavo svetlo](projekti/treptavo-svetlo.md)
- [Zujalica](projekti/zujalica.md)

## Sopstvena struja

- [Solarne ćelije](solarne-celije.md)
- [Generator](generator.md)
- [Regulator napona](regulator-napona.md)
- [Baterije za solarni sistem](baterije-za-solarni-sistem.md)
- [Punjenje baterija](punjenje-baterija.md)
- [Invertor](invertor.md)
- [Punjenje laptopa](punjenje-laptopa.md)

### Literatura:

- Vladimir Krstić, *Mala škola elektronike*, Beograd, 1968.
- Miomir Filipović, *Elektronika za početnike*, internet izdanje.
- Gordon McComb i Earl Boysen, *Elektronika za neupućene*, Beograd 2006.
- Ian Sinclair, John Dunton *Practical Electronics Handbook*, Newnes 2007.
- Baiano Reeves, *Solar Power DIY Handbook - So, You Want To Connect Your Off-Grid Solar Panel to a 12 Volts Battery*, web izdanje, 2018.
